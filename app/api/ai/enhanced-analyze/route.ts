...
export async function POST(request: NextRequest) {
  try {
    ...
    if (followUpQuery) {
      try {
        ...
        const { data: originalAnalysisData, error: analysisError } = await (serviceSupabase as any)
          ...
          .eq('analysis_type', 'enhanced_business_analysis')
    ...
    // Check for existing enhanced analysis first (unless forced refresh)
    let analysis;
      ...
      try {
        ...
        if (!cacheError && cached) {
          ...
          if (optionsMatch) {
            ...
            console.log('Using cached enhanced analysis from', cached.created_at);
          }
        }
      } catch (error) {
        console.log('No cached enhanced analysis found, generating fresh analysis');
      }
    }
    ...
    if (!existingAnalysis || forceRefresh) {
      ...
      try {
        ...
        if (!insertError) {
          console.log('✅ Enhanced business analysis saved to database');
        } else {
          console.error('Failed to save enhanced business analysis:', insertError);
        }
      } catch (error) {
    ...
    console.error('Error in enhanced AI business analysis:', error);
